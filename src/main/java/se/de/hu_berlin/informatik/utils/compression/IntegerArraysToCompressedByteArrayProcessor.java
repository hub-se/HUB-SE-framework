 ...
 * @author Simon Heiden
 */
public class IntegerArraysToCompressedByteArrayProcessor extends AbstractProcessor<Integer[],byte[] > {
	...
	public IntegerArraysToCompressedByteArrayProcessor(int maxValue, boolean containsZero) {
	 ...
	 */
	@Override
	public byte[] processItem(Integer[] intArray) {
		if (sequenceLength == 0) {
	...
	private static byte ceilLog2(int n) {
	    ...
	    if (n == 0) {
	    	Log.warn(IntegerArraysToCompressedByteArrayProcessor.class, "Maximum input number is zero.");
	    	return 1;
	    } else {
